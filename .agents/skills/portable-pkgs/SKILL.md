...
Use `--dry-run --format json` only when the inferred repo, target assets, or archive paths need inspection before editing the manifest. Ordinary successful adds are easy to review and revert through the manifest diff.
