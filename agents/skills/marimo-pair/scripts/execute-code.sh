...
# Build optional auth header. This is needed both for direct execution and for
# probing authenticated local servers that are absent from marimo's registry.
auth_args=()
if [[ -n "$token" ]]; then
...
if [[ -n "$url" ]]; then
  ...
  probe_marimo_http() {
    local host="$1"
    local probe_port="$2"
    local probe_base="http://${host}:${probe_port}"

    curl -sf --max-time 0.5 "${probe_base}/health" >/dev/null 2>&1 || return 1

    local version=""
    version=$(curl -sf "${auth_args[@]+"${auth_args[@]}"}" --max-time 0.5 "${probe_base}/api/version" 2>/dev/null || true)
    if [[ -z "$version" || "$version" != *.* ]]; then
      local sessions_status
      sessions_status=$(curl -s -o /dev/null -w '%{http_code}' "${auth_args[@]+"${auth_args[@]}"}" --max-time 0.5 "${probe_base}/api/sessions" 2>/dev/null || true)
      [[ "$sessions_status" == "200" || "$sessions_status" == "401" || "$sessions_status" == "403" ]] || return 1
    fi

    jq -n \
      --arg server_id "127.0.0.1:${probe_port}" \
  ...
  if [[ $count -eq 0 ]]; then
    if [[ -n "$port" ]]; then
      ...
      for probe_port in $(list_listening_ports); do
        [[ "$probe_port" =~ ^[0-9]+$ ]] || continue
        e=$(probe_marimo_http "127.0.0.1" "$probe_port" 2>/dev/null || true)
        [[ -n "$e" ]] || continue
        entry="$e"
        count=$((count + 1))
      done
  ...
  if [[ $count -eq 0 ]]; then
    echo "No running marimo instances found in the registry or on local listening ports." >&2
    exit 1
  fi

  if [[ $count -gt 1 ]]; then
    echo "Multiple marimo-like instances found. Use --port or --url to specify." >&2
    exit 1
  fi
