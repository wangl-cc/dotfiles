...
if type -q get_bg # {{{
    function autobg --on-event fish_prompt
        ...
        if test "$bg" != "$__system_current_bg"
            ...
            if test "$__system_current_bg" = Dark
                emit autobg_switch_to_dark
            else
                emit autobg_switch_to_light
            end
        end
    end
    ...
    if test -n "$KITTY_LISTEN_ON"
        ...
        function kitty_colors_dark --on-event autobg_switch_to_dark
            kitty @set-colors -a -c "$__kitty_theme_dir/tokyonight_moon.conf"
        end
        function kitty_colors_light --on-event autobg_switch_to_light
            kitty @set-colors -a -c "$__kitty_theme_dir/tokyonight_day.conf"
        end
    end

    if type -q bat
        function bat_theme_dark --on-event autobg_switch_to_dark
            set -gx BAT_THEME tokyonight_moon
        end
        function bat_theme_light --on-event autobg_switch_to_light
            set -gx BAT_THEME tokyonight_day
        end
    end

    if type -q delta
        function delta_features_dark --on-event autobg_switch_to_dark
            set -gx DELTA_FEATURES tokyonight-moon
        end
        function delta_features_light --on-event autobg_switch_to_light
