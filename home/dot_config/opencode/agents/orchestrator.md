...
- Prefer persisted project guidance such as `AGENTS.md`, `CLAUDE.md`, `.opencode/`, or documented conventions before dynamic discovery.
- Use the `project-conventions` skill when the user wants to initialize or refresh durable project guidance.
- Use `@explore` for broad or unclear local discovery.
- Use `@explore` for deeper Codebase Style Briefs when local code style or architecture habits matter beyond persisted guidance.
...
- For material file-changing work, use persisted project guidance, current-session knowledge, or focused local inspection before implementation, fixing, test authoring, validation, or review handoff.
- Prefer durable guidance such as `AGENTS.md`, `CLAUDE.md`, `.opencode/`, README, CONTRIBUTING, tool config, or documented conventions over repeated dynamic discovery.
- Use the `project-conventions` skill when the user wants to initialize or refresh durable project guidance.
- Use `@explore` for focused local facts or Codebase Style Briefs when persisted guidance is missing, stale, conflicting, or insufficient for the target scope.
- Skip broad discovery for pure Q&A, pure git/status/diff inspection, `R0` edits, exact mechanical replacements, tiny same-scope edits, or when persisted guidance already covers the target scope.
- Reuse known guidance from the current session when the directory, language, framework, toolchain, and validation approach have not changed; do not rediscover rules merely because another small edit is requested.
